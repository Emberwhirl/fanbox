...
`playwright-electron.js` puts a fake `claude` (`claude.cmd` → Node shim) on the tab PATH; it reads the `--settings` file FanBox typed and posts the same HTTP-hook events Claude Code sends, so the whole chain (flag → file → header interpolation → `/api/agent/event` → renderer) is exercised without an Anthropic login. Results of the 2026-09-04 run are in `docs/win-port-implementation-notes-v2.16.1.html`.
