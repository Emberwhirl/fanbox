...
window.FANBOX_DICT = {
  ...
  '已关闭 · 恢复正常休眠': 'Off · normal sleep resumes',
  ...
  '开启后：只要检测到有 agent 正在干活，Windows 会尽量不进入睡眠，任务接着跑；所有终端都空闲约两分钟后，自动恢复正常休眠。笔记本合盖是否睡眠仍看「电源选项 → 合上盖子的操作」。':
    'When any agent is actively working, Windows tries not to sleep so the task keeps going. Once every terminal has been idle for ~2 minutes, normal sleep resumes. Lid-close sleep still follows Power Options → “When I close the lid”.',
  ...
  '自动保存 · Ctrl+S 立即保存': 'Autosave · Ctrl+S to save now',
  '源码': 'Source',
  '富文本': 'Rich text',
  '插入图片': 'Insert image',
  '排版…': 'Typeset…',
  '上移': 'Move up',
  '下移': 'Move down',
  '选择要插入的图片': 'Choose images to insert',
  '图片': 'Images',
  '只读 · 此文件富文本往返有损，要改请点源码': 'Read-only · rich text round-trip would lose syntax — edit in Source',
  '只读 · 要改请点富文本或源码': 'Read-only · switch to Rich text or Source to edit',
  '选图失败：': 'Could not pick images: ',
  '图片复制失败：': 'Could not copy image: ',
  '插入图片失败：': 'Could not insert image: ',
  '图片写盘失败': 'Failed to save image',
  '该环境不支持图片写盘': 'This environment cannot save images to disk',
  '粘贴图片转存失败：': 'Could not persist pasted image: ',
  '图片写盘失败：': 'Failed to save image: ',
  '复制到公众号': 'Copy for WeChat',
  '送去… ▾': 'Send to… ▾',
  '处理图片': 'Processing images',
  '生成长图…': 'Exporting long image…',
  '切分节…': 'Slicing sections…',
  '已导出': 'Exported',
  '导出失败': 'Export failed',
  '复制失败': 'Copy failed',
  '已复制': 'Copied',
  ...
  'GitHub Release（附 exe）': 'GitHub Release (with exe)',
  ...
  '终端铺满（Ctrl+Alt+M）': 'Maximize terminal (Ctrl+Alt+M)',
  '还原终端（Ctrl+Alt+M）': 'Restore terminal (Ctrl+Alt+M)',
  '在资源管理器显示': 'Reveal in File Explorer',
  '复制文件（资源管理器里可粘贴）': 'Copy file (paste in File Explorer)',
  ...
  '只把这一个文件退回本回合开工前（git 项目退回 HEAD）；本回合新建的文件会移入回收站': 'Revert just this file to before this round (HEAD in git projects); files created this round go to Recycle Bin',
  '还原中…': 'Reverting…',
  '已还原': 'Reverted',
  ...
  '已下载并打开安装包，按提示完成覆盖安装': 'Downloaded and opened the installer — follow the prompts to finish',
...
window.FANBOX_DICT_RULES = [
  ...
  [/^现在：(\d+) 个终端开着，agent 正在干活 → 生效中，尽量不睡眠$/, (m) => `Now: ${m[1]} terminal(s) open, agents working → active, trying not to sleep`],
  [/^现在：(\d+) 个终端开着但都空闲 → 系统照常休眠$/, (m) => `Now: ${m[1]} terminal(s) open but all idle → normal sleep`],
  ...
  [/^把「(.+)」退回本回合开工前的版本？之后对它的改动会被移除（本回合新建的文件会移入回收站，可找回）$/, (m) => `Revert "${m[1]}" to before this round? Later changes to it will be removed (files created this round go to the Recycle Bin, recoverable)`],
  [/^「(.+)」已还原$/, (m) => `"${m[1]}" reverted`],
  [/^「(.+)」是本回合新建的，已移入废纸篓$/, (m) => `"${m[1]}" was created this round — moved to Trash`],
  [/^「(.+)」是本回合新建的，已移入回收站$/, (m) => `"${m[1]}" was created this round — moved to Recycle Bin`],
  [/^批注 (.+)$/, (m) => `Note on ${m[1]}`],
