...
- Claude Code one-click launch on Windows appends `--settings` with a quoted absolute HTTP hook file under `~/.fanbox/hooks/` (token only in process env / headers). FanBox-launched Codex is `pty.spawn`'d with a real argv (`-c notify=[…]` as one slot; never typed into PowerShell). Terminal maximize is `Ctrl+Alt+M`.
